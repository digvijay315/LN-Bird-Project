...
const[projects,setprojects]=useState(
  ...
  const addRow = () => {
    setRows([...rows, { id: rows.length + 1, image: '', previewUrl: '' }]);
  };
  ...
  const handleImageChange = (e, index) => {
    ...
    reader.onloadend = () => {
      ...
      newRows[index].previewUrl = reader.result;
      setRows(newRows);
    };
    ...
    return rows.map((row, index) => (
      <tr key={row.id}>
        ...
        <td>
          ...
          {row.previewUrl && (
            <img src={row.previewUrl} alt={`Preview ${index + 1}`} style={{ maxWidth: '100px', maxHeight: '100px' }} />
          )}
        </td>
        <td>
        ...
        <td>
        <div className="col-md-12"><select className="form-control" required="true" onChange={(e)=>settowers({...towers,project:e.target.value})}>
        ...
        <td style={{width:"20px",cursor:"pointer"}}>
          <img src="https://t4.ftcdn.net/jpg/03/46/38/39/360_F_346383913_JQecl2DhpHy2YakDz1t3h0Tk3Ov8hikq.jpg" alt="delete button" onClick={() => deleteRow(index)}/>
        </td>
      </tr>
    ));
  };
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <table style={{marginLeft:"25px"}}>
                 <thead >
                  ...
                  <div className="row mt-4">
                  <div className="col-md-2" style={{marginLeft:"80%"}} onClick={addRow}><button className="form-control">Add Link Url</button></div>
                </div>
                <div className="d-flex justify-content-between align-items-center mb-3">
