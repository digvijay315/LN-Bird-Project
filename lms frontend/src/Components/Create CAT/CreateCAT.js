...
const MCQQuestion = ({ onSave, mainSkill, subSkills }) => {
  ...
  const validateQuestions = () => {
    ...
    const totalQuestions = subSkillSections.reduce(
    ...
    if (totalQuestions < 5) {
      errorMessage = 'Please add atleast 5 minimum question';
      isValid = false;
    }

    if (totalQuestions > 20) {
      errorMessage = 'Please add atmost 20 minimum question';
      isValid = false;
    }

    subSkillSections.forEach(section => {
      if (section.questions.length > 0 && !section.selectedSubSkill) {
        errorMessage = 'Please select a sub-skill for all sections with questions';
        isValid = false;
      }

      section.questions.forEach(question => {
        if (!question.question.trim()) {
          errorMessage = 'All questions must have text';
          isValid = false;
        }

        if (!question.category.main || !question.category.sub) {
          errorMessage = 'Please select main and sub category for all questions';
          isValid = false;
        }
        ...
        if (question.points < 1 || question.points > 10) {
          errorMessage = 'Points must be between 1 and 10';
          isValid = false;
        }
      });
    });
